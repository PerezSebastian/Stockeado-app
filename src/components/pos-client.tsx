        ...
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
            ...
            <div className="flex-1 flex flex-col space-y-4 min-w-0">
                <div>
                    ...
                    <p className="text-zinc-500">Seleccioná los productos y registrá la venta.</p>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 overflow-y-auto pr-1 pb-4">
                        {filtered.map((p) => {
                            ...
                            return (
                                <Card
                                    key={p.id}
                                    className={`transition-all border-2 bg-white ${outOfStock
                                        ? "opacity-50 border-zinc-100 cursor-not-allowed select-none"
                                        : inCart
                                            ? "cursor-pointer border-zinc-900 shadow-md"
                                            : "cursor-pointer border-zinc-200 hover:border-zinc-400 hover:shadow-sm"
                                        }`}
                                ...
                                >
                                    <CardContent className="p-3 flex flex-col justify-between h-full space-y-3">
                                        ...
                                        <div className="flex items-end justify-between gap-1 flex-wrap">
                                            ...
                                            {outOfStock ? (
                                                <Badge className="text-[10px] bg-amber-50 text-amber-600 border border-amber-200 hover:bg-amber-50">
                                                    Sin Stock
                                                </Badge>
                                            ) : (
            ...
            <div className="w-full lg:w-[380px] flex flex-col bg-white rounded-xl border border-zinc-200 overflow-hidden shadow-sm shrink-0">
                {/* Header ticket */}
                <div className="p-4 bg-zinc-50/80 border-b border-zinc-200 flex justify-between items-center">
                    ...
                    {cartCount > 0 && (
                        <span className="text-xs font-semibold bg-zinc-900 text-white px-2 py-0.5 rounded-full">
                            {cartCount}
                        </span>
                    )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {cart.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-zinc-400 space-y-2">
                            ...
                            <p className="text-xs text-center">Hacé clic en un producto para agregarlo</p>
                        </div>
                    ) : (
                        cart.map((item) => (
                            <div key={item.id} className="flex gap-3 border-b border-zinc-100 pb-3">
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium leading-tight text-zinc-800 truncate">
                                        {item.name}
                                    </p>
                                ...
                                <div className="flex flex-col items-end justify-between shrink-0">
                                    ...
                                    <div className="flex items-center gap-1 mt-1.5">
                                        ...
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            className="h-6 w-6 rounded text-red-400 hover:text-red-600 hover:bg-red-50 ml-0.5"
                ...
                <div className="p-4 bg-zinc-50 border-t border-zinc-200 space-y-4">
                    ...
                    <Button
                        ...
                        className="w-full h-12 text-base font-semibold bg-zinc-900 hover:bg-zinc-800 text-white shadow-md"
                    >
            ...
            <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
                <DialogContent className="sm:max-w-[420px]">
                    ...
                    <div className="space-y-3 py-2">
                        <div className="bg-zinc-50 rounded-lg p-3 space-y-1.5 max-h-40 overflow-y-auto">
                            {cart.map((item) => (
                        ...
                        <div className="flex items-center justify-between px-1">
                            <span className="text-zinc-500 text-sm">Total a cobrar</span>
