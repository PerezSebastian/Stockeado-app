                    ...
                    {data.map((item, index) => (
                        <div key={index} className="space-y-1.5 flex flex-col">
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="text-success font-bold">${Number(item.revenue).toLocaleString("es-AR")}</span>
                            </div>
                            <div className="relative h-2 w-full overflow-hidden rounded-full bg-surface-subtle">
                                <div
                                    className="h-full bg-success/150 rounded-full transition-all duration-1000 ease-in-out"
